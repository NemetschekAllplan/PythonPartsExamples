...
def create_element(build_ele, doc):
    ...
    try:
        ...
        print("build_ele.ColumType.value: ", build_ele.ColumType.value)
        ...
        refPoint = AllplanGeo.Point3D(0, -50, 500) # ref point (origin)

        holeGeometry = AllplanGeo.BRep3D.CreateCylinder(AllplanGeo.AxisPlacement3D(
                                                            refPoint,
                                                            AllplanGeo.Vector3D(1, 0, 0), # x-axis
                                                            AllplanGeo.Vector3D(0, 1, 0)), # z-axis
                                                        cylRad, cylHeight)

        columnProps.AddHole(holeGeometry);


        refPoint = AllplanGeo.Point3D(0, -50, 600) # ref point (origin)
