...
class ClosedStirrupFreeform():
    ...
    def create_profile(self):
        ...
        if self.horizontal_placement:
            profile += AllplanGeo.Point3D(self.bottomwidth + delta, 0, self.height)
            profile += AllplanGeo.Point3D(- delta, 0, self.height)
        else:
            profile += AllplanGeo.Point3D(self.bottomwidth + delta, self.height, 0)
            profile += AllplanGeo.Point3D(- delta, self.height, 0)
        ...
        if self.horizontal_placement:
            ...
            self.handle_list.append(
                HandleProperties("TopWidth",
                                 AllplanGeo.Point3D(self.bottomwidth + delta, self.placement_length, self.height),
                                 AllplanGeo.Point3D(-delta, self.placement_length, self.height),
            ...
            self.handle_list.append(
                HandleProperties("TopWidth",
                                 AllplanGeo.Point3D(self.bottomwidth + delta, self.height, self.placement_length),
                                 AllplanGeo.Point3D(-delta, self.height, self.placement_length),
                                 [("TopWidth", HandleDirection.x_dir)],
                                 HandleDirection.x_dir,
                                 True))
