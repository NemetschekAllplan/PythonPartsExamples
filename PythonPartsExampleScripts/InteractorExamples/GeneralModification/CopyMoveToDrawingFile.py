...
if TYPE_CHECKING:
    from __BuildingElementStubFiles.CopyMoveToDrawingFileBuildingElement import CopyMoveToDrawingFileBuildingElement
else:
    CopyMoveToDrawingFileBuildingElement = BuildingElement


print('Load CopyMoveElements.py')
...
class CopyElements(BaseInteractor):
    ...
    def __init__(self,
        ...
        self.build_ele      = cast(CopyMoveToDrawingFileBuildingElement, build_ele_list[0])
    ...
    def process_mouse_msg(self,
        ...
        #----------------- copy / moove the elements

        build_ele = self.build_ele

        target_DF = build_ele.TargetDrawingFile.value

        if (target_DF != AllplanBaseEle.DrawingFileService.GetActiveFileNumber()):

            if (build_ele.Command.value == "Copy"):
                AllplanBaseEle.CopyElementsToDrawingFile(self.coord_input.GetInputViewDocument(), self.selected_elements, 
                                                     build_ele.TargetDrawingFile.value, self.coord_input.GetViewWorldProjection())
            elif (build_ele.Command.value == "Move"):
                AllplanBaseEle.MoveElementsToDrawingFile(self.coord_input.GetInputViewDocument(), self.selected_elements, 
                                                     build_ele.TargetDrawingFile.value, self.coord_input.GetViewWorldProjection())
        else:
            Util.ShowMessageBox("Please select another drawing file than the active one as target",Util.MB_OK)
